...
def cit_analy(threadname,fname,qid,parentDoc,sid):
	...
	if not s.find('&REFID=')==-1:
		...
		if ref_status=='OK':
			cur_cit = esi_cit_analy(threadname,fname,qid,parentDoc,py,sid)
	return ut,py,cur_cit

def esi_cit_analy(threadname,fname,qid,parentDoc,py,sid):
	s = file(fname, 'r').read()
	hrefstr = findstr(s,'"CAScorecard_count_WOSCLASSIC">','</span>')
	href = findstr(hrefstr,'href="','" name=')
	cur_cit = '0'
	if len(href)>0:
		p0 = href.find('?action=AllCitationService')
		url_esi_cit = 'http://apps.webofknowledge.com/InterService.do'+href[p0:]
		fname = './'+threadname+'/temp/esi_ref_list.html'
		esi_ref_status = openurl(url_esi_cit, sid,None, fname)
		if esi_ref_status=='OK':
